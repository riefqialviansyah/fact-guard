...
class UserController {
  ...
  static async saveNewUser(req, res) {
    ...
    try {
      ...
      console.log(error, "<<<<<<< other errror");
    }
  }
  ...
  static async saveProfile(req, res) {
    let UserId = req.session.UserId;
    try {
      let { firstName, lastName, dateOfBirth, hobby, gender, organization } =
        req.body;
  ...
  static async editProfile(req, res) {
    ...
    try {
      ...
      await Profile.update(
        { firstName, lastName, dateOfBirth, hobby, gender, organization },
        {
          where: {
            id: ProfileId,
          },
  ...
  static async addTag(req, res) {
    ...
    try {
      if (!TagId) {
        throw { name: "required", message: "Please select tag to add" };
      }
  ...
  static async cetak(req, res) {
    try {
      ...
      allUser.forEach((el) => {
        space += 10;
        doc.text(
          `${el.dataValues.id} -> ${el.dataValues.username} -> ${el.dataValues.role} -> ${el.dataValues.password}`,
          20,
          space
        );
      });
